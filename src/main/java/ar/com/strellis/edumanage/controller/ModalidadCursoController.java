...
{
	static Logger log = LogManager.getLogger(CursosController.class);
	@Autowired
	...
	{
		ModalidadCurso m=null;
		try {
			m = modalidadCursoService.getById(id);
			...
			@RequestParam(value="tipo_curso_id",required=true) Integer tipo_curso_id)
	{
		...
		return new LinkedList<ModalidadCurso>();
	}
