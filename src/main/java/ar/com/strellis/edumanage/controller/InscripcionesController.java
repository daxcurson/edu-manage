...
import ar.com.strellis.edumanage.model.validation.ModeloFormInscripcionValidator;
...
{
	static Logger log = LogManager.getLogger(InscripcionesController.class);
	...
	public @ResponseBody List<Curso> listarCursos(@RequestParam("idioma_estudiar_id") int idiomaEstudiarId,@RequestParam("nivel_id") int nivelId)
	{
		// Busco los cursos.
		// Tengo que localizar los idiomas y niveles referidos!
		Nivel nivel=null;
		IdiomaEstudiar idiomaEstudiar=null;
		try {
			nivel = nivelService.getById(nivelId);
			idiomaEstudiar=idiomaEstudiarService.getById(idiomaEstudiarId);
		...
		return cursoService.listarCursosIdiomaNivel(idiomaEstudiar, nivel);
	}
	/**
	...
	public ModelAndView cargarFormConfirmarInscripcion(int inscripcion_id)
	{
		...
		try {
			inscripcion = this.inscripcionService.getById(inscripcion_id);
