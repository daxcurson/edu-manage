...
import ar.com.strellis.edumanage.documentation.NombreClase;
import ar.com.strellis.edumanage.documentation.Permiso;
...
{
	static Logger log = LogManager.getLogger(PermisosController.class);
	...
	{
		ModelAndView modelo=new ModelAndView("permisos_index");
		return modelo;
	}
	...
	{
		ModelAndView modelo=new ModelAndView("permisos_listar_permisos");
		...
		Map<NombreClase,List<Permiso>> controllers=new LinkedHashMap<NombreClase,List<Permiso>>();
		...
		for(Map.Entry<RequestMappingInfo, HandlerMethod> entry:p.entrySet())
		{
			...
			if(!controllers.containsKey(n))
			{
				...
				List<Permiso> metodos=new LinkedList<Permiso>();
	...
	public @ResponseBody String informarPermiso(@RequestParam("permiso") String permiso)
	{
		...
		try
		{
			...
			Optional<Group> grupoOpcional=permissionService.findGroupById(Integer.parseInt(id));
			Group grupo=grupoOpcional.orElseThrow(GrupoInexistenteException::new);
	...
	public @ResponseBody String modificarPermiso(@RequestParam("permiso") String permiso)
	{
		...
		try
		{
			...
			String id=t.nextToken();
			String permisoBuscar=t.nextToken();
			Group grupo=permissionService.findGroupById(Integer.parseInt(id)).orElseThrow(GrupoInexistenteException::new);
			if(grupo!=null)
			{
