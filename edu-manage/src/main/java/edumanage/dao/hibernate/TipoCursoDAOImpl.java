...
public class TipoCursoDAOImpl implements TipoCursoDAO 
{
	...
	{
		return (List<TipoCurso>) sessionFactory.getCurrentSession().createQuery("from TipoCurso").list();
	}

	@Override
	public TipoCurso getById(long parseLong) 
	{
		return (TipoCurso) sessionFactory.getCurrentSession().createQuery("from TipoCurso where id="+parseLong).uniqueResult();
	}
}
