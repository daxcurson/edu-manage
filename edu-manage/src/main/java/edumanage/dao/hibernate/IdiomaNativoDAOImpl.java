...
public class IdiomaNativoDAOImpl implements IdiomaNativoDAO
{
	...
	{
		return (List<IdiomaNativo>) sessionFactory.getCurrentSession().createQuery("from IdiomaNativo").list();
	}

	@Override
	public IdiomaNativo getById(long id) 
	{
		return (IdiomaNativo) sessionFactory.getCurrentSession().createQuery("from IdiomaNativo where id="+id).uniqueResult();
	}
